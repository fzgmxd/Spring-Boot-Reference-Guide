...
```
这项配置将会根据数据库的类型设置使用的文件夹（比如MySQL对应的是`db/migration/mysql`），而不是使用`db/migration`。支持的数据库的一览在[`DatabaseDriver`](https://github.com/spring-projects/spring-boot/tree/v2.0.0.M2/spring-boot/src/main/java/org/springframework/boot/jdbc/DatabaseDriver.java).
